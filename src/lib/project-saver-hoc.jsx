...
const ProjectSaverHOC = function (WrappedComponent) {
    class ProjectSaverComponent extends React.Component {
        ...
        createNewProjectToStorage () {//cloud storage
        ...
        storeProject(projectId, requestParams) {
            try {
              ...
              return Promise.all(this.props.vm.assets
                ...
                .map(asset => {
                  ...
                  .then(response => {
                    ...
                    if (response.status !== 'ok') {
                      ...
                      console.error('Error occurred while storing asset. Response code:', response.code);
                      return Promise.reject(response.code);
                    }

                    asset.clean = true;
                    console.log('Asset marked as clean:', asset);
                  })
                  .catch(err => {
                    console.error('Error caught while storing asset:', err);
                    throw err; // pass the error up the chain
                  })
                }))
                .then(() => {
                  console.log('All assets saved successfully. Updating project data with ID:', projectId);
                  return this.props.onUpdateProjectData(projectId, savedVMState, requestParams);
                })
                .then(response => {
                  console.log('Project data updated successfully. Update response:', response);
                  this.props.onSetProjectUnchanged();
                  console.log('Project marked as unchanged');

                  const id = response.id.toString();
                  if (!id) {
                    console.error('Project ID missing from response. Unable to update project thumbnail.');
                    return Promise.reject('Project ID missing from response.');
                  }

                  console.log('Response ID converted to string:', id);

                  if (this.props.onUpdateProjectThumbnail) {
                    console.log('Updating project thumbnail with ID:', id);
                    return this.storeProjectThumbnail(id)
                      .then(() => {
                        console.log('Project thumbnail updated successfully');
                        return response;
                      })
                      .catch(err => {
                        console.error('Error occurred while updating project thumbnail:', err);
                        throw err; // pass the error up the chain
                      });
                  }
                ...
                .then(response => {
                  console.log('Reporting telemetry event');
                  this.reportTelemetryEvent('projectDidSave');
                  console.log('Project saved successfully');
                  return response;
                })
                .catch(err => {
                  console.error('Error caught while saving project:', err);
                  log.error(err);
                  throw err; // pass the error up the chain
                });
            } catch (err) {
