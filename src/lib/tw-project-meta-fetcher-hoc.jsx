...
export const fetchProjectMeta = async projectId => {
    const urls = [
        `https://localhost:8080/backend/projectsMETA.php?${projectId}`,
        `https://localhost:8080/backend/projectsMETA.php?${projectId}`
    ];
    let firstError;
    for (const url of urls) {
        try {
            ...
            if (res.ok) {
                const canSaveLoaded = typeof data.canSave === 'string' ? data.canSave.toLowerCase() === 'true' : false;
                return {
                    ...data,
                    canSaveLoaded
                };
        ...
        componentDidUpdate () {
            ...
            fetchProjectMeta(projectId)
                .then(data => {
                    ...
                    /*if(canSaveLoaded === "true") {
                        canSaveLoaded = true;
                    }else if(canSaveLoaded === "false") {
                        canSaveLoaded = false;
                    }else{
                        canSaveLoaded = false;
                    }*/
