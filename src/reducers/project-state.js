...
const getIsCreatingNew = loadingState => (
    new URLSearchParams(location.search).get('new_project') === '1' &&
...
const doneCreatingProject = (id, loadingState) => {
    if (new URLSearchParams(location.search).get('new_project') === '1') {
        console.log('manual bypass (highly dangerous)')
        window.location.href = window.location.origin + "/projects/" + id.toString();
